...
function VoiceFlappyGame({ startLevel = 1, onGameEnd }) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (gameStateRef.current === 'playing') {
        ...
        // เมื่อเงียบ ตกเร็วขึ้นชัดเจน — responsive มากขึ้น
        const gravity = smoothVol < 0.04 ? 0.06 : 0.02;
        bird.vy += gravity;
        bird.vy -= lift;
        bird.vy = Math.max(-1.5, Math.min(bird.vy, 5));
        bird.y += bird.vy;
        ...
        if (scoreRef.current >= lv.target) {
          ...
          const winDur = Math.round((Date.now() - startTimeRef.current) / 1000);
          if (onGameEndRef.current) onGameEndRef.current({
            win: true, stars: 3, level: currentLevelRef.current,
            metrics: { pipes_passed: scoreRef.current, duration_secs: winDur },
          });
        }
        if (dead) {
          ...
          const deadDur = Math.round((Date.now() - startTimeRef.current) / 1000);
          if (onGameEndRef.current) onGameEndRef.current({
            win: false, stars: finalSc >= 7 ? 2 : 1, level: currentLevelRef.current,
            metrics: { pipes_passed: finalSc, duration_secs: deadDur },
          });
        }
      }
  ...
  const startGame = (level) => {
    ...
    pipesRef.current = [];
    scoreRef.current = 0;
    frameRef.current = LEVELS[level].pipeInterval - 80; // ท่อแรกมาใน ~80 เฟรม
